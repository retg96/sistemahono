...
<?php
  if(isset($_POST['add_user'])){
   ...
   $req_fields = array('full-name','username','password','level','subdireccion','hijo');
   ...
   if(empty($errors)){
       ...
       $password = sha1($password);
        ...
        $query .="name,clave,password,nivel_usuario,status,IdSubdireccion,IdHijo";
        $query .=") VALUES (";
        $query .=" '{$name}', '{$username}', '{$password}', '{$user_level}','1','$subdireccion','$hijo'";
        $query .=")";
        if($db->query($query)){
          //sucess
          $session->msg('s'," El usuario ha sido creado con exito");
  ...
  <div class="row col-md-12">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <div class="col-md-12">
          <form novalidate method="post" action="add_user.php">
            ...
            <div class="form-group">
                          <label>Hijo:</label>
								            <select class="form-control" name="hijo" required>
									            <?php 
                              $result=$db->query('SELECT * FROM hijo');
                                while($f=mysqli_fetch_array($result)) {
                              ?>
									            <option value="<?php echo $f['id'] ?>">
                              <?php echo $f['Hijo'] ?></option>
									            <?php } ?>
							          	</select>      
            </div>
